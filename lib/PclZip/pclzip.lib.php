...
class PclZip
{
    ...
    // Big5 目錄
    var $big5Folder = array();
    var $otherFolder = array();
...
//        echo '<pre>';
//        var_dump('組織欲刪除 Big5 目錄，KEY為目錄深度');
//        var_dump($this->big5Folder);
//        echo '</pre>';
        // 組織欲刪除 Big5 目錄，KEY為目錄深度
        $rmFolderSeq = array();
        foreach ($this->big5Folder as $v) {
//            echo '<pre>';
//            var_dump(mb_substr_count($v, '/'));
//            echo '</pre>';
            $rmFolderSeq[mb_substr_count($v, '/')][] = $v;
        }

        foreach ($this->otherFolder as $v) {
            $files = scandir($v);
            $count = 0;
            foreach ($files as $f) {
                if ($f!='.' && $f!='..') $count++;
...
//        echo '<pre>';
//        var_dump($rmFolderSeq);
//        echo '</pre>';
        // 依照目錄深度，由深到淺排序，需先刪除深度深的，不然會回報失敗
        krsort($rmFolderSeq);
//        echo '<pre>';
//        var_dump($rmFolderSeq);
//        echo '</pre>';
        ...
        // 刪除 Big5 目錄
        foreach ($rmFolderSeq as $k) {
            foreach ($k as $v) {
//                echo '<pre>';
//                var_dump(preg_match('/^\/home\/[^\.][a-zA-Z0-9_\/]*\/base\/\d{5}\/course\/\d{8}\//', $v));
//                var_dump($v);
//                echo '</pre>';

                // 稽核路徑，不可有 .
                if (preg_match('/^\/home\/[^\.][a-zA-Z0-9_\/]*\/base\/\d{5}\/course\/\d{8}\//', $v) === 1 && is_dir($v)) {
...
//                        define('WP_MEMORY_LIMIT', '128M');
        ...
        // 更名流程
//        echo '<pre>';
//        var_dump($p_entry['filename']);
//        var_dump($p_entry['stored_filename']);
//        echo '</pre>';
        if (is_dir($p_entry['filename'])) {
            $p_entry['stored_filename'] = mb_substr($p_entry['stored_filename'], 0, -1);
        }
        $contentPathParse = explode('/', $p_entry['stored_filename']);
        $contentPath = array();
        $isChange = '0';
        foreach ($contentPathParse as $v) {
//            echo '<pre>';
//            var_dump($v);
//            var_dump(iconv('BIG5', 'UTF-8', $v));
//            var_dump(iconv('UTF-8', 'BIG5', iconv('BIG5', 'UTF-8', $v)));
//            var_dump(mb_detect_encoding($v, 'auto'));
//            echo '</pre>';
            // 如果是 Big5
            if (mb_strlen(iconv('BIG5', 'UTF-8', $v)) >= 1 && mb_detect_encoding($v, 'auto') !== 'ASCII' && mb_detect_encoding($v, 'auto') !== 'UTF-8' && $v === iconv('UTF-8', 'BIG5', iconv('BIG5', 'UTF-8', $v))) {
...
//        echo '<pre>';
//        var_dump(implode('/', $contentPath));
//        echo '</pre>';
        
        // 進行更名
        if ($isChange === '1') {
            if (is_dir($p_entry['filename'])) {
                $oldname = substr($p_entry['filename'], 0, -1);
//                echo '<pre>';
//                var_dump('big5Folder');
//                var_dump($oldname);
//                echo '</pre>';
                $this->big5Folder[] = $oldname;
            } else {
                $oldname = $p_entry['filename'];
            }
            $newname = $p_path . '/' . implode('/', $contentPath);
//            echo '<pre>';
//            var_dump('rename---->');
//            var_dump($oldname, $newname);
//            var_dump(pathinfo($newname, PATHINFO_DIRNAME));
//            echo '</pre>';
            @mkdir(pathinfo($newname, PATHINFO_DIRNAME), 0777, true);
            @rename($oldname, $newname);

            if (!in_array(pathinfo($oldname, PATHINFO_DIRNAME),$this->otherFolder) && pathinfo($oldname, PATHINFO_DIRNAME)!=$p_path)
            $this->otherFolder[] = pathinfo($oldname, PATHINFO_DIRNAME);
