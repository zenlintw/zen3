...
// ***************************************************************** 
function Folder(folderDescription, hreference, is_js) //constructor 
{ 
  //constant data 
...
/*
  if(is_js==undefined)
  	...
  	alert(folderDescription + '\nis_js = ' + is_js);
*/
  this.is_js = (is_js==undefined?false:is_js);
  this.id = -1;
  this.navObj = 0;
  this.iconImg = 0; 
  this.nodeImg = 0;
...
{ 
  ...
  if (browserVersion > 0) 
    auxEv = "<a href='javascript:void(null);' onClick='clickOnNode(\""+this.getID()+"\");return false;'>" 
  else 
    auxEv = "<a>" 
...
{ 
  ...
  { 
	if(this.is_js) {
		docW = docW + "<a href='javascript:void(null)' ";
	} else {
		if (USEFRAMES) {
    ...
    if (browserVersion > 0) {
	if(this.is_js)
		docW = docW + "onClick='clickOnFolder(\""+this.getID()+"\");" + this.hreference + "; return false;' ";
	else 
...
{ 
  ...
  this.createIndex = createEntryIndex;
  this.forceOpeningOfAncestorFolders = forceOpeningOfAncestorFolders;
  this.setActive = setItemActive;

  finalizeCreationOfItem(this)
} 

// Add by Yang 2004/06/09
function setItemActive(v)
{
  this.active = v;
  this.iconSrc = (v?(ICONPATH + "doc.gif"):(ICONPATH + "disdoc.gif"));
}
...
function finalizeCreationOfItem(itemArray)
{
  ...
  itemArray.iconSrc = ICONPATH + "doc.gif" 
...
function gFld(description, hreference, is_js) 
{ 
  folder = new Folder(description, hreference, is_js);
  return folder;
} 
