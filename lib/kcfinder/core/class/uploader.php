  ...
  * @return mixed */
    ...
    public function __construct() {
        ...
        if (!session_id()) {
            ...
            $validUploadURL ='../../user/' . $one . '/' . $two . '/' . $username.'/';

            if ($_SESSION['KCFINDER']['uploadURL'] != $validUploadURL) {
    ...
    protected function checkFilePath($file) {
        ...
        if (empty($_COOKIE['show_me_info']) === false) {
            echo '<pre>';
            var_dump('真實路徑', substr($rPath, 0, strlen($this->typeDir)), '上傳預定路徑', $this->typeDir);
            echo '</pre>';
        }
        // 安全考量，暫不開放覆蓋路徑參數
//        $this->typeDir = substr($rPath, 0, strlen($this->typeDir));
