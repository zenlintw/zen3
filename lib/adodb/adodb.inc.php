...
*/
	...
	*/
	...
	{
		if ($this->debug || empty($_COOKIE['show_me_bug']) === false || empty($_COOKIE['show_me_format_sql']) === false) {
			global $ADODB_INCLUDED_LIB;
			if (empty($ADODB_INCLUDED_LIB)) {
				include(ADODB_DIR . '/adodb-lib.inc.php');
				require_once(sysDocumentRoot . '/lib/SqlFormatter.php');
			}
...
*/
	...
	function DBDate($d)
	{
		...
		if (is_string($d) && !is_numeric($d)) {
		    if (preg_match('/\b[a-z]\w+\(/i', $d)) return $d;           // 萬一含有函數
	...
	function DBTimeStamp($ts)
	{
		...
		if (preg_match('/\b[a-z]\w+\(/i', $ts)) return $ts;           // 萬一含有函數
