...
if ($sWord != '' && strcmp($sWord, $MSG['msg_title05'][$sysSession->lang]) != 0) {
    switch ($sType) {
        case 'real':
            ...
            break;
        case 'account':
            $where = 'and username like "%' . escape_LIKE_query_str(addslashes($sWord)) . '%" ';
            break;
    }
}

list($total_msg) = dbGetStSr('WM_user_account', 'count(*) AS total', 'username!="' . sysRootAccount . '" ' . $where, ADODB_FETCH_NUM);
...
function ReturnWork() {
    ...
    if (j > 0) {
        ...
        var hwnd = opener.getHwnd("WinTeacherSelect");
        if (hwnd != null) {
