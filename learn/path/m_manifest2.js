...
function xmlProcessor()
{
	...
	if (myPanel.getElementsByTagName('li').length <= 0) myPanel.innerHTML = '<h4 style="text-align: center;">'+MSG_NO_DATA+'</h4>';
...
function generateOrganizationByXsl(node)
{
	...
	xslDoc.load('m_manifest2.xsl.php'+slang);
...
function generateOrganization(node){
	var htmlCode = '', ret = '', idRef = '', nodeTitle = '', htmlTitle = '', nodeID = '', hasRef = false, isDisable = false, newTarget, nodeDuration = '';
	var iref, res, href;
	if (node === null) return '';
	window.status += '|';
	...
	*/
	...
	for(var i=0; i<nodes.length; i++){
        ...
        if ((ret = generateOrganization(nodes[i])) == '') {
			htmlCode += '';
			if (hasRef && !isDisable)
				htmlCode += '<li id="' + nodeID + '" class="" onclick="return launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" title="'+htmlTitle+'">' +
                                '<div class="point"></div>' +
                                '<div class="data" style="width: 190px;">' +
                                '<div class="section">' + MSG_ASSET_ORDER.replace('%num%', (i+1)) + '</div>' +
                                '<div class="title">' + nodeTitle + '</div>' +
                                ((nodeDuration != '')?'<div class="time">'+nodeDuration+'</div>':'');
			else if (hasRef && isDisable)
				htmlCode += '<li id="' + nodeID + '" class="" onclick="return launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" disabled title="'+htmlTitle+'">' +
                                '<div class="point"></div>' +
                                '<div class="data" style="width: 190px;">' +
                                '<div class="section">' + MSG_ASSET_ORDER.replace('%num%', (i+1)) + '</div>' +
                                '<div class="title">' + nodeTitle + '</div>' +
...
*/
    ...
    setTimeout(function(){
        $('#displayPanel li').prop("class", "");
        // 取得節點狀態
        getLearnRecord();
        // 修改節點已讀狀態(目前先採用看過即完成)
        $.each(learnRecord.path.item, function(i, v){
            if (v.readed === true) {
                $('#displayPanel').find("#" + v.identifier).prop("class", "finish");
            }
        });
        ...
        $("#progressBar", parent.document).css("width", aPercent+"%");
        $("#progressBar-text", parent.document).text(aPercent+"%");

        // 已完成的單元顯示自我評量按鈕
        if (aPercent == 100 && asmtNum != 0) {
            $("#self-assessment", parent.document).show();
        }
...
// 顯示自我評量
function showExam() {
    var nId = learnRecord.path.item[0].identifier;
    nId = nId.replace("I_", "");
    window.open("/learn/path/lcms.php?motion=exam&type=a&rid="+nId+"&num="+asmtNum, "lcmsDialog",  "height=" + screen.height + ", width=" + screen.width + "resizable=1,toolbar=0,menubar=0,scrollbars=1,status=0");
}