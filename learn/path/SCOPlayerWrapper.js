...
function doLMSGetValue(name)
{
   ...
   if (getValueReturn == null)
   {
      alert('null cause : ' +  name);
      return '';
   }
   else if(typeof(getValueReturn) == 'undefined')
   {
	  alert('undefined cause : ' +  name);
      return '';
   }
   else if (getValueReturn == 'en-US')
      return 'US/English'; // add for SCORM 1.2
   else if (getValueReturn.search(/^P(T([0-9]+H)?([0-9]+M)?([0-9]+(\.[0-9]{1,2})?S)?)?$/) === 0)
      return getValueReturn.replace(/[HM]/g, ':').replace(/[PTS]/g, '').replace(/:([0-9]):/, ':0$1:').replace(/^([0-9]):/, '0$1:').replace(/:([0-9])(.[0-9]+)?$/, ':0$1$2').replace(/\.00$/, '');
   else if (getValueReturn == 'true_false' || getValueReturn == 'fill_in')
      return getValueReturn.replace('_', '-');
   else if (getValueReturn == 'multiple_choice')
      return 'choice';
   else if (getValueReturn == 'not_attempted')
      return 'not attempted';
   else if (getValueReturn == '' && elementRequestArr[tempArrCount] == '_count')
   {
      api.LastError = 201;
...
function doLMSSetValue(name, value)
{
	if (value == 'US/English') value = 'en-US'; // add for SCORM 1.2
   ...
   if ( name.search(/\.(_children|_count|_version)$/) > -1 )
   {
      api.LastError = 402;
      _InternalErrorCode = API_CALL_PASSED_TO_LMS;
      return 'false';
   }
   else if ( elementRequestArr[0] == "cmi" )
   {
...
function translateDataModelElement(name)
{
   ...
   case "interactions":
      {
         DataModelElementReturnVal = convertInteractions(name,arrayOfComponents);
         return DataModelElementReturnVal;
      }
   case "comments":
...
function convertObjectives(name,arrayOfComponents)
{
   ...
   if ( arrayOfComponents[3] == "status" )
   {
      ...
      {
         _InternalErrorCode = API_CALL_PASSED_TO_LMS;
         objReturnValue = api.GetValue(objectivesStatusRequestArr[arrayOfComponents[2]]);
      }
   }
   else
   {
...
function convertInteractions(name,arrayOfComponents)
{
   ...
   if ( arrayOfComponents[3] == "time" )
   {
      ...
      var result1 = api.GetValue("cmi.interactions." + arrayOfComponents[2] + ".timestamp");
...
function dmElementSetFunction(name, value)
{
   ...
   switch(arrayOfComponents[arrayOfComponents.length-1])
   {
   case 'id':
   ...
   case 'min':
   case 'max':
       value = value.toString(); // custom by chiahua redmine#4137
       if (value.substr(0,1) == '-')
       {
 	     api.LastError = 201;
 	     _InternalErrorCode = API_CALL_PASSED_TO_LMS;
   ...
   case "interactions":
      {
      	 ...
      	 {
      	 	if (value == 'true-false' || value == 'fill-in')
   ...
   case "comments":
      {
         _InternalErrorCode = API_CALL_PASSED_TO_LMS;
         setReturnValue = api.SetValue(setNameUpdate, api.GetValue(setNameUpdate) + value);
         return setReturnValue;
      }
   default:
      {
      	 if ((setNameUpdate == 'suspend_data' || setNameUpdate == 'comments') && value.length > 4096)
      	 {
            _InternalErrorCode = API_CALL_PASSED_TO_LMS;
            api.LastError = 406;
            return 'false';
...
function setConvertCore(name,value,arrayOfComponents)
{
   ...
   {
   	  if (value.search(/^[0-9]{2,4}(:[0-9]{2}){2}(\.[0-9]{1,2})?$/) !== 0)
   	  {
   	  	  api.LastError = 406;
   	  	  _InternalErrorCode = API_CALL_PASSED_TO_LMS;
   	  	  return 'false';
   ...
   {
   	   if (value.search(/^(time-out|suspend|logout|)$/) !== 0)
   	   {
         _InternalErrorCode = API_CALL_PASSED_TO_LMS;
         api.LastError = 406;
         return 'false';
   	   ...
   	   {
   	   	 if (value == 'time-out') value = 'timeout';
         _InternalErrorCode = API_CALL_PASSED_TO_LMS;
...
function setConvertObjectives(name,value,arrayOfComponents)
{
   ...
   if ( arrayOfComponents[3] == "status" )
   {
      if ( (value == "passed") || (value == "failed") )
      {
         ...
         objectivesFlag = "";
         _InternalErrorCode = API_CALL_PASSED_TO_LMS;
         objReturnValue = api.SetValue("cmi.objectives." + arrayOfComponents[2] + ".success_status", value);
         objectivesStatusRequestArr[arrayOfComponents[2]] = "cmi.objectives." + arrayOfComponents[2] + ".success_status";
         return objReturnValue;
      }
      else if ( (value == "completed") || (value == "incomplete") || (value == "not attempted") )
      {
         // Reset Objectives Flag
         objectivesFlag = "";
         _InternalErrorCode = API_CALL_PASSED_TO_LMS;
         objReturnValue = api.SetValue("cmi.objectives." + arrayOfComponents[2] + ".completion_status", value.replace(' ', '_'));
         objectivesStatusRequestArr[arrayOfComponents[2]] = "cmi.objectives." + arrayOfComponents[2] + ".completion_status";
         return objReturnValue;
      }
      else if ( value == "browsed")
      {
...
function setConvertInteractions(name,value,arrayOfComponents)
{
   ...
   {
   	  if (value.search(/^[0-9]{2,4}(:[0-9]{2}){2}(\.[0-9]{1,2})?$/) == -1)
   	  {
   	  	  api.LastError = 406;
   	  	  _InternalErrorCode = API_CALL_PASSED_TO_LMS;
   	  	  return 'false';
   ...
   {
   	  if (value.search(/^[0-2][0-9](:[0-5][0-9]){2}(\.[0-9]{1,2})?$/) == -1)
   	  {
   	  	  api.LastError = 406;
   	  	  _InternalErrorCode = API_CALL_PASSED_TO_LMS;
   	  	  return 'false';
      ...
      _InternalErrorCode = API_CALL_PASSED_TO_LMS;
      var result1 = api.SetValue("cmi.interactions." + arrayOfComponents[2] + ".timestamp", newValue);
      return result1;
   }
   else if ( arrayOfComponents[3] == "result" )
   {
