...
<![CDATA[
    ...
    function getUnitCnt(lang) {
        return '{$MSG['msg_asset_order'][$sysSession->lang]}'.replace('%num%', (cnt++));
    }
		...
		<xsl:if test="not(@isvisible) or @isvisible != 'false'">
			...
			<li>
                ...
                <div class="data" style="width: 190px;">
                    ...
                    <xsl:variable name="duration" select="concat(/manifest/resources/resource[@identifier=current()/@identifierref]/@xml:base, /manifest/resources/resource[@identifier=current()/@identifierref]/@duration)"/>
                    <xsl:if test=" (\$duration != 'undefined') and (\$duration != '00:00:00') ">
                        <div class="time">
