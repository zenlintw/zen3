...
function xmlProcessor()
{
    ...
    // 取得上一次離開的節點，若 沒有最後一次的節點 或 不存在 或 節點被設定隱藏
        if (globalCurrentActivity == '' || organization.selectSingleNode('//item[@identifier="' + globalCurrentActivity + '"]') == null || organization.selectSingleNode('//item[(@identifier="' + globalCurrentActivity + '") and (not(@isvisible) or @isvisible!="false") and (not(@disabled) or @disabled!="true")]') === null){
...
var traceReadingIntervalTime = 60 * 1000;
...
*/
        ...
        if( typeof xmlDoc.load == "undefined" ){
            ...
            xmlHttp.open('GET', '/learn/path/SCORM_loadCA.php'+ser, xmlDoc.async);
            xmlHttp.send();
            var    ret    = xmlHttp.responseText;
            xmlDoc.loadXML(ret);
        }else{
            xmlDoc.load('/learn/path/SCORM_loadCA.php'+ser);
        }
...
//        setTimeout('xmlProcessor()', 3000);
    ...
    {
        if (!xmlDoc.load('/learn/path/SCORM_loadCA.php'+ser)){
...
function generateOrganizationByXsl(node)
{
    ...
    xslDoc.load('/learn/path/manifest.xsl.php'+slang);
    ...
    */
    ...
    for(var i=0; i<nodes.length; i++){
        ...
        if ((ret = generateOrganization(nodes[i])) == '') {
            htmlCode += '<li id="' + nodeID + '"><span>' +
                        '<div class="icon-node"></div>';
                        // '<img src="' + themePath + 'icon-ccc.gif" valign="absmiddle" border="0">&nbsp;' ;
                        
            if (hasRef && !isDisable)
                htmlCode += '<a href="javascript:;" onclick="return launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" class="cssAnchor1" title="'+htmlTitle+'">' + nodeTitle + '</a>';
            else if (hasRef && isDisable)
                htmlCode += '<a href="javascript:;" onclick="return launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" disabled style="cursor:default" title="'+htmlTitle+'">' + nodeTitle + '</a>';
            else
                htmlCode += nodeTitle;
            ...
            if (hasRef && !isDisable)
                htmlCode += '<a href="javascript:;" onclick="expanding(this.previousSibling.previousSibling);return launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" title="'+htmlTitle+'"><span class="node-folder">' + nodeTitle + '</span></a>';
            else if (hasRef && isDisable)
                htmlCode += '<a href="javascript:;" onclick="expanding(this.previousSibling.previousSibling);return launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" disabled style="cursor:default" title="'+htmlTitle+'"><span class="node-folder">' + nodeTitle + '</span></a>';
            else
...
function launchActivity(obj,id,target){
    ...
    var nodeTitle = getTitle(item);
    var htmlTitle = escapeHtml(stripTags(nodeTitle));
     ...
     */
    if (resource != null && id == globalCurrentActivity && obj != null && !isone)
    {
        if (resource.getAttribute('scormtype') != 'sco')
        {
            ...
            if (!rr.test(href))
            {
                ...
                if (target == '_blank') document.s_main.location = 'about:blank';
                objForm.submit();
                fetchServerTime();
            }
        }
        ...
        // 取得此節點的title
        $('#material-node-title').html(htmlTitle);

        // 隱藏學習路徑
        showPathtree(false);
        isLaunching = false;
        isYoutubeMovie = (resource.getAttribute('type') == 'youtube')?true:false;
...
function nextStep(dir){
    ...
    switch(dir){
        case 1:    // 往子代找，找不到往弟代找，找不到往叔代找...................................
            ...
            if (node != null)
            {
                clearTimeout(timerStart); timerStart = setTimeout("setReading('start', 0);", pathNodeTimeShortlimit * 1000); 
...
function getLearnRecord() {
    $.ajax({
        url: '/xmlapi/index.php?action=my-course-path-info&onlyProgress=1&cid='+cid+'&ticket='+pTicket,
        datatype: 'json',
        async: false,
...
function setReading(type, period) {
    ...
    console.log('setReading', type, period);
...
//    if (window.console) {console.log('title', $(objForm).find("input[name='prev_node_title']").val().replace(/\"/g,""));}
...
//    if (window.console) {console.log('activity_id', $(objForm).find("input[name='prev_node_id']").val());}  
    ...
    if (type === 'start') {
        if (window.console) {console.log('set prev_node_id', 'selected', $('.step-process2').find("li[class='selected']").prop('id'), 'globalCurrentActivity',globalCurrentActivity);}
        $(objForm).find("input[name='prev_node_id']").val(globalCurrentActivity);
    }
