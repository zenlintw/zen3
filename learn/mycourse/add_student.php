...
**/
	...
	if (!isset($_REQUEST['HTTP_RAW_POST_DATA']) && isset($GLOBALS['HTTP_RAW_POST_DATA'])) {
		...
		*/
		...
		// get course name
		list($cour_name, $n_limit) = dbGetStSr('WM_term_course','caption, n_limit','course_id=' . $course_id, ADODB_FETCH_NUM);
		$cour_lang = unserialize($cour_name);

		// 判斷是否超過正式生人數限制
		list($n_cnt) = dbGetStSr('WM_term_major', 'count(*)', 'course_id='. $course_id . ' and role & ' . $sysRoles['student'], ADODB_FETCH_NUM);
		if ($n_limit && $n_cnt >= $n_limit) {
			...
			if (empty($role)){
				$RS = dbNew('WM_term_major', 'username,course_id,role,add_time', "'" . $sysSession->username . "'," . $course_id . ',' . $sysRoles['student'] . ',NOW()');
				...
				else if ($role & $sysRoles['auditor']) {	// 已經是旁聽生，轉為正式生
				    $mask = $sysRoles['all'] ^ $sysRoles['auditor'];
				    dbSet('WM_term_major', "role=role & $mask | {$sysRoles['student']},add_time=NOW()", "username='{$sysSession->username}' and course_id={$course_id}");
				    if ($sysConn->ErrorNo() == 0) {
				        $state_msg = $MSG['add_success'][$sysSession->lang];
				        $state_msg = str_replace('%COURSE_NAME%',$cour_lang[$sysSession->lang],$state_msg);
				        $state_code = 0; // success
				...
				else if ($role & $sysRoles['teacher']) {	// 教師則可以加入正式生
					$mask = $sysRoles['all'] ^ $sysRoles['auditor'];
					dbSet('WM_term_major', "role=role & $mask | {$sysRoles['student']},add_time=NOW()", "username='{$sysSession->username}' and course_id={$course_id}");
					if ($sysConn->ErrorNo() == 0) {
						$state_msg = $MSG['add_success'][$sysSession->lang];
