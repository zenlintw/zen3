<?php
	...
	$sysSession->restore();
	if (!aclVerifyPermission(1710400300, aclPermission2Bitmap('enable,visible,readable,writable,modifiable,uploadable,removable'))){
	}
...
function viewExemplar(examinee, ticket) {
	...
	$rs = dbGetStMr('WM_qti_peer_result', 'examinee', sprintf('exam_id=%u and time_id=%d and status="publish" order by score DESC', $_SERVER['argv'][0], $_SERVER['argv'][1]), ADODB_FETCH_ASSOC);
	if (!$rs) {
	...
	showXHTML_tabFrame_B(array(array($MSG['exemplary_list'][$sysSession->lang])));
		...
		if (isAssignmentForGroup($_SERVER['argv'][0]))  // 如果是群組作業
		{
			...
			$sqls = 'select distinct D.group_id,G.caption ' .
					...
					'left join WM_qti_peer_result as R ' .
