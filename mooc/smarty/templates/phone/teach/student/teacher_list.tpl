                    ...
                    <table class="table subject">
                        ...
                        <td class="">
                            ...
                            {if $self_level eq 'teacher'}
                                ...
                                &nbsp;
                                {else}
                                <button name="btnDelAssistant" type="button" class="btn" onclick="doRemoveAssistant('{$v.username}','{$v.level}');">{'delete'|WM_Lang}</button>
                                {/if}
                            {else}
                            &nbsp;
                            {/if}
...
<script type="text/javascript">
    var msgEmptyTeacherString = "{'title12'|WM_Lang}";
    var msgConfirmDelete = "{'title41'|WM_Lang}";
    {literal}
    function checkData() {
        var obj = document.getElementById("actForm");

        if (obj.username.value == ''){
            alert(msgEmptyTeacherString);
            obj.username.focus();
            return false;
        }
        ...
        $.ajax({
            'type': 'POST',
            'dataType': 'json',
            'data': {'action' : 'addAssistant', 'username' : obj.username.value, 'level' : obj.level.value},
    ...
    function doRemoveAssistant(username, level) {
        if (confirm(msgConfirmDelete)){
            $.ajax({
                'type': 'POST',
                'dataType': 'json',
                'data': {'action' : 'removeAssistant', 'username' : username, 'level' : level},
                'url': '/mooc/controllers/course_ajax.php',
                'success': function (response) {
                    if (response == 'ok'){
                        alert('刪除完成。');
