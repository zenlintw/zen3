...
<div class="box1" style="width:400px;">
    ...
    <div class="content">
		...
		{if $reviewRuleId <= 1}
		  <button class="btn btn-blue" onclick="add_student();">{'add_audit'|WM_Lang}</button>
		{else}
		  <button class="btn btn-blue" onclick="doSendElective({$course_id});">{'add_audit1'|WM_Lang}</button>
		{/if}
...
<script type="text/javascript">
    {$inlineJS}
    {literal}
    function doSendElective(csid) {
        var courses = [];
        courses.push(csid);
        obj = parent.getTarget();
        if ((obj != null) && (typeof(obj.do_func) == "function")) {
            obj.do_func('major_add', courses);
            obj.do_func('elective', courses);
