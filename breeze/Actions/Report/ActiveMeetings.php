...
class ActiveMeetingXML
{
	...
	function ActiveMeetingXML($xml)
	{
		...
		$this->urlpath = parseValue("<url-path>/(.*)/</url-path>",$xml);
		$this->date_begin = parseValue("<date-begin>(.*)</date-begin>",$xml);
	}
}

class ActiveMeetings extends Action
{
	var $result = array();
	function ActiveMeetings($sess)
	{
		parent::action('report-active-meetings', $sess);
	}
...
function getActiveMeetingList($sess, $idx)
{
	$action = new ActiveMeetings($sess);
	$action->run();
	$rtnArray = array();
