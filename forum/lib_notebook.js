	...
	function parseListFolder(node, indent) {
		...
		for (var i = 0; i < cnt; i++) {
			...
			if (nodes[i].nodeName == "folder") {
				...
				name = htmlspecialchars(getCaption(nodes[i]));
...
///////////////////////////////////////////////////////////////////////////////
	...
	function showManageNBFolder() {
		...
		indx = 0;
		col = "bg03";
		txt  = '<table width="100%" border="0" cellspacing="1" cellpadding="3" class="box01">';
		txt += '<tr class="bg02"><td class="font01" noWrap="noWrap">';
		txt += MSG_HELP;
		txt += '</td></tr>';
		txt += '<tr class="bg03"><td class="font01" noWrap="noWrap">';
	...
	function do_func(act, extra) {
		...
		switch (act) {
			case 'list_folder'   :
				...
				xmlHttp.open("POST", "/forum/nb_dir.php", false);
