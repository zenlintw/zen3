...
function go_rowspage(n){
	...
	_goto('500,'+board_id+','+ new_page +','+sortby+'.php');
}

function go_page(n){
	switch(n){
		case -1:	// 第一頁
			_goto('500,'+board_id+',1,'+sortby+'.php'); // location.replace
			break;
		case -2:	// 前一頁
			_goto('500,'+board_id+','+(cur_page-1)+','+sortby+'.php');
			break;
		case -3:	// 後一頁
			_goto('500,'+board_id+','+(cur_page+1)+','+sortby+'.php');
			break;
		case -4:	// 最末頁
			_goto('500,'+board_id+','+total_page+','+sortby+'.php');
			break;
		case '0':	// 全部列
			_goto('501,'+board_id+','+cur_page+','+sortby+'.php');
			break;
		default:	// 指定某頁
			_goto('500,'+board_id+','+n+','+sortby+'.php');
			break;
	}
}
...
function do_notebook( folder) {
	if(folder != null)
		location.replace('531,'+board_id+','+from_post+','+to_post+'.php?folder='+folder);
	from_post = 0;
	to_post = 0;
}
...
function do_q_folder( folder , folder_id) {
	if(folder_id != null && q_folder_type != '') {
		if(q_folder_type == 'copy')
			location.replace('532,'+board_id+','+from_post+','+to_post+'.php?folder_id='+folder_id);
		else if(q_folder_type == 'move')
			location.replace('533,'+board_id+','+from_post+','+to_post+'.php?folder_id='+folder_id);
	}
	from_post = 0;
	to_post = 0;
	q_folder_type = '';
...
function batch(obj){
	...
	switch(obj.value){
		case '1':	// 刪除
			...
			location.replace('530,'+board_id+','+v1+','+v2+'.php');
			break;
		case '2':	// 收入個人筆記本
		...
		case '4':	// 移入 (move)
			...
			showDialog("batch_q_folder.php?board="+ board_id+"&v1="+v1+"&v2=" + v2,
				true,window, true, 0, 0, '300px', '350px', 'resizable=yes;scroll=no;status=no;');
...
function sortBy(type){
	...
	location.replace('502,'+board_id+','+cur_page+','+ta[type]+'.php');
}

function post(){
	location.replace('write.php?bTicket=' + bTicket);
}
