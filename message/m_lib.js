...
///////////////////////////////////////////////////////////////////////////////
	...
	function gpExpand(val) {
		...
		if (obj.style.display == "block") {
			...
			if (icon != null) {
				icon.src = "/theme/" + theme + "/minus.gif";
...
//            console.log(nodes);
            for (var i = 0; i < cnt; i++) {
                ...
                if (nodes[i].nodeName == "folder") {
                    ...
                    createtime = nodes[i].getAttribute("createtime");
                    
                    // 暫時關閉，用不到
//                    if (idx !== 'sys_notebook_trash') {
//                        txt +=  '<div class="item">' +
//                                    '<div class="tabs">' +
//                                        '<div class="tab">'+
...
//                                                '<div class="title">' + getCaption(nodes[i]) + '</div>' +
//                                                '<div class="total">' + msg.total[nowlang] + '3' + msg.items[nowlang] + '</div>' +
//                                            '</div>' +
//                                        '</div>' +
//                                    '</div>' +
//                                    '<div class="operate">' +
...
//                console.log(node);
	...
	function displaySetPage(idex) {
		...
		for (i = 0; i < langList.length; i++) {
			...
			obj = document.getElementById("GPName_" + langList[i]);
			if (obj != null) {
				obj.value = getCaption(nodes[editIdx]);
			}
		}
		lang = orgLang;
		// get node name (End)
