<?php
	...
	require_once(sysDocumentRoot . '/lang/CO_links_manage.php');
	require_once(sysDocumentRoot . '/lang/co_lang_manage.php');

	//引入表單後端動作程式
	require_once('links_handler.php');
	...
	switch ($_POST['act']) {
	    ...
	    case 'sa':
	        $updateSuccess='';
	        $errorCode=updateAdvSec(intval(trim($_POST['links_sec'])));
	        if ($errorCode=='') {
	            $updateSuccess=true;
	        } else {
	            $updateSuccess=false;
	...
	function showAdvName($name, $advId) {
		return '<div style="white-space:normal;word-wrap:break-word" id="links_name_'.$advId.'" title="'.$name.'">'.$name.'</div>';
	}
	...
	function showImage($val) {
		if (empty($val)) return '&nbsp;';
		$imgPath = realpath(sysDocumentRoot.'/base/10001/door/'.$val);
		if (file_exists($imgPath)) {
			return sprintf('<img src="/base/10001/door/%s" style="width:160px;height:64px;" />', $val);
		}
		return '&nbsp;';
	}
	/**
	...
	function showAction($advId, $url) {
		...
		return '<input type="button" value="' . $MSG['btn_modify'][$sysSession->lang] .
               '" class="cssBtn" onclick="location=\'links_edit.php?edit=1&links_id='.$advId.'\'"/> ' .
	...
	function setAdvSec(){
        ...
        if (parseInt(obj.links_sec.value)>20 || parseInt(obj.links_sec.value)==0) {
            alert('{$MSG['int_0_to_20_errMsg'][$sysSession->lang]}');
            return false;
        }

	    obj.links_sec.value=obj.links_sec.value;
	...
	function chgTab(tabId) {
		if (tabId == 1) {
			document.location.href = '/academic/co_adv/adv_manage.php';
		}
	}
EOB;
	...
	showXHTML_body_B();
			$ary = array(
					array($MSG['manage_title'][$sysSession->lang], 'tabsSet2',  '')
				);
			echo '<div align="center">';
			showXHTML_tabFrame_B($ary, 2, 'mainFm', '', 'action=""  method="post" style="display:inline;" target="resWin"');
			...
			// 工具列
			$toolbar = new toolbar();
            $toolbar->add_input('button', '', $MSG['btn_add'][$sysSession->lang]  , '', 'class="cssBtn" onclick="location=\'links_edit.php\'" id="btn_new"');
			$toolbar->add_input('button', '', $MSG['btn_rm'][$sysSession->lang]  , '', 'class="cssBtn" onclick="rmConfirm(\'mainFm\', \'links_name\');" ');
			...
			$toolbar->add_input('button', 'sv', $MSG['btn_save'][$sysSession->lang],   '', 'class="button01" onclick="savePermute(\'mainFm\', \'nodeids\', \'links\')"');
			...
			$ck2 = new toolbar();
			$ck2->add_input('checkbox', 'nodeid[]'  , '%links_id', '', 'onclick="chgCheckbox(); event.cancelBubble=true;"');
			...
			$myTable->add_field($MSG['links_name'][$sysSession->lang]  , '', ''    , '%1 %0'   , 'showAdvName'   , 'width="160px"');
			...
			$myTable->add_field($MSG['img_path'][$sysSession->lang], '', '', '%5', 'showImage' , 'nowrap="noWrap"');
			$myTable->add_field($MSG['btn_modify'][$sysSession->lang]    , '', '', '%0 %4' , 'showAction'  , 'align="center" nowrap="noWrap"' );

			$tab    = 'CO_links';
			$fields = 'links_id, name, open_date, close_date, url, img_path, permute, open_date_flag, close_date_flag';
			$where  = '1 order by permute asc, name asc';
        ...
        if ($updateSuccess==true) {
            showXHTML_script('inline', "alert('".$MSG['update_success'][$sysSession->lang]."')");
        }
	showXHTML_body_E();
?>
