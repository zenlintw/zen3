<?php
	...
	switch ($_POST['act']) {
	    ...
	    case 'add':
	        ...
	        if ($errorCode=='') {
    	        ...
    	        if ($errorCode=='') {
    	            header('location: links_manage.php');
    	            exit;
    	        }
	        }
	        break;
	    ...
	    case 'update':
	        ...
	        if ($errorCode=='') {
                $errorCode=updateData($_POST['links_id'], $formData, $_FILES['upload']);
    	        if ($errorCode=='') {
    	            header('location: links_manage.php');
    	            exit;
    	        }
	...
	//若操作動作為修改時，執行資料讀取
	if( $_GET['edit']=='1') {
	    $advId=intval(trim($_GET['links_id']));
	    if ($advId>0) {
	        $data=dbGetStSr('CO_links', '*', 'links_id='.$advId, ADODB_FETCH_ASSOC);
	...
	var MSG_URL_ERROR ="{$MSG['check_url_error'][$sysSession->lang]}";

	/**
	 *將資料放入表單
	 */
	function setValue(links_id, open_date, close_date, img_path, open_date_flag, close_date_flag, action){
	    var obj =document.getElementById('newForm');
    	if ((obj != null) && (obj != 'undefined')){
    	    if (parseInt(links_id)>0) {
    ...
    function submitCheck(){
        ...
        if (trim(obj.name.value)=='') {
            ERRORMSG+="{$MSG['empty_links_name'][$sysSession->lang]}"+"\\r\\n";
            result= false;
        ...
        if (!IsURL(obj.url.value)) {
            ERRORMSG+=MSG_URL_ERROR+"\\r\\n";
            result= false;
        } else if (getTxtLength(obj.url.value)>1000) {
...
EOB;
	...
	echo '<div align="center">';
		...
		showXHTML_tabFrame_B($ary, 1, 'newForm', 'newTable', 'action="links_edit.php" method="POST" enctype="multipart/form-data"  style="display:inline"');
		  showXHTML_table_B('border="0" cellpadding="3" cellspacing="1" class="cssTable"');
		    ...
		    showXHTML_tr_B('class="cssTrEvn"');
		      showXHTML_td('align="right"', $MSG['links_name_label'][$sysSession->lang].'<span style="color:red">*</span>');
		      showXHTML_td_B();
			...
			showXHTML_tr_B('class="cssTrOdd"');
		      showXHTML_td('align="right"', $MSG['url_label'][$sysSession->lang].'<span style="color:red">*</span>');
		      showXHTML_td_B();
		    ...
		    showXHTML_tr_B('class="cssTrEvn"');
		      showXHTML_td_B('colspan="3" align="center"');
		        ...
		        showXHTML_input('button', '', $MSG['cancel'][$sysSession->lang], '', 'class="cssBtn" onclick="location=\'links_manage.php\'"');
		      showXHTML_td_E();
		    showXHTML_tr_E();
		    showXHTML_input('hidden', 'links_id', '', '', '');
		    if(is_array($data) && count($data)>0) {
		        showXHTML_input('hidden', 'act', 'update', '', '');
		        showXHTML_input('hidden', 'img_path', '', '', '');
        	    showXHTML_script('inline', "setValue('".$data['links_id']."',  '".$data['open_date']."', '".$data['close_date']."', '".$data['img_path']."', '".$data['open_date_flag']."', '".$data['close_date_flag']."')");
        	} elseif($_POST['act']==='update' || $_POST['act']==='add') {
        	    showXHTML_input('hidden', 'act', $_POST['act'], '', '');
        	    showXHTML_script('inline', "setValue('".$_POST['links_id']."',  '".$_POST['open_date']."', '".$_POST['close_date']."',  '".$_POST['img_path']."', '".$_POST['ck_open_date']."', '".$_POST['ck_close_date']."', '".$_POST['act']."')");
        	} else{
