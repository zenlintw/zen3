<?php
	...
	$sqls = 'select B.log_time FROM APP_log_others as B where LOCATE("App User Login", B.`note`)';
...
EOF;
	...
	showXHTML_body_B('');
		...
		showXHTML_table_B('align="center" width="500" border="0" cellspacing="0" cellpadding="0" id="ListTable"');
			showXHTML_tr_B('');
				showXHTML_td_B('');
					$ary   = array();
					...
					showXHTML_table_B('id ="mainTable" width="500" border="0" cellspacing="1" cellpadding="3" class="cssTable"');
							...
							switch ($_POST['type_report']){
								...
								case 5:		/* 連續日報表 add by wiseguy for TODO#1269 */
                                    $sqls .= sprintf(' and B.log_time BETWEEN "%s 00:00:00" AND "%s 23:59:59"', $_POST['daily_from_date'], $_POST['daily_over_date']);
                                    $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
									...
									showXHTML_tr_B('class="cssTrEvn"');
										showXHTML_td_B('align="center" colspan="2"');
											echo $MSG['from'][$sysSession->lang], $_POST['daily_from_date'], $MSG['to'][$sysSession->lang], $_POST['daily_over_date'],
											str_replace(array('%TOTAL_PEOPLE%'),
								...
								case 2:		// 週報表
									...
									if ($rs && $rs->RecordCount() > 0){	// if begin
                                        if (is_array($_POST['login']) && in_array('3',$_POST['login']) )
										    $condition_of_3s = dbGetCol('WM_term_major',
																		'username',
																		'1 GROUP BY username HAVING count(*) ' . $_POST['state_sel'] . max(0, $_POST['total_course']));
		...
		showXHTML_tabFrame_B($ary, 1, 'exportForm', 'exportTable', 'action="app_sch_cour_export.php" method="POST" style="display: inline" target="empty"', true);
		  showXHTML_table_B('border="0" cellpadding="3" cellspacing="1" class="cssTable"');
		    ...
		    showXHTML_tr_B('class="cssTrOdd"');
		      ...
		      showXHTML_td_B();
                $downloadFilename = 'app_login_stat_'.date('Ymd',time()).'.zip';
		        showXHTML_input('text', 'download_name', $downloadFilename, '', 'maxlength="60" size="40" class="box02"');
		      showXHTML_td_E();
		    showXHTML_tr_E();
		    showXHTML_tr_B('class="cssTrEvn"');
