...
");
	...
	showXHTML_body_B();
		echo "<center>\n";
		$ary = array(array($MSG['title_pack'][$sysSession->lang]));
		showXHTML_tabFrame_B($ary, 1, 'actForm', 'ListTable', 'method="POST" action="course_pack1.php" onsubmit="if (this.course_id.value.search(/^[0-9]{8}$/) == -1) { alert(\'' . $MSG['msg_course_id_error'][$sysSession->lang] . '\'); return false;} else this.elements[this.elements.length-1].disabled=true;" style="display: inline"');
				showXHTML_table_B('id ="mainTable" width="680" border="0" cellspacing="1" cellpadding="3" class="cssTable"');
					...
					showXHTML_tr_B('class="font01 cssTrEvn"');
					  showXHTML_td('width="80"', $MSG['th_course_id'][$sysSession->lang]);
					...
					showXHTML_tr_B('class="font01 cssTrOdd"');
					  showXHTML_td('', $MSG['th_course_package'][$sysSession->lang]);
					  showXHTML_td_B();
					    showXHTML_input('radio', 'package_how', array('download'  => $MSG['msg_package_download'][$sysSession->lang],
					    											  'putindoor' => $MSG['msg_package_putindoor'][$sysSession->lang]), 'download', '', '<br>');
					  showXHTML_td_E();
					...
					showXHTML_tr_B('class="font01 cssTrEvn"');
					  ...
					  echo <<< EOB
  <ul>
    <li><img src="/theme/{$sysSession->theme}/teach/icon-c.gif" border="0" align="absmiddle" onclick="expand_collapse(this);" style="cursor: pointer; cursor: hand"><input type="radio" value="standard" checked name="package_detail" id="fp1"><label for="fp1">{$MSG['rd_package_standard'][$sysSession->lang]}</label>
