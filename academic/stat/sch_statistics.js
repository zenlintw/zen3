/**
 * 學校統計資料
 * $Id: sch_statistics.js,v 1.1 2010/02/24 02:38:43 saly Exp $
*/
	function do_fun(val){
        //alert(val);
//		if ( (1 <= parseInt(val)) && (parseInt(val) <= 7) ) {
        if ( (1 <= parseInt(val)) && (parseInt(val) <= 8) ) {
		...
		switch (parseInt(val)){
			case 1:	 	// 開課統計 (course)
				...
				break;
			case 2:		// 登入次數統計 (login)
				parent.main.location.href = "sch_login_statistics.php";
				break;
			...
			case 7:		// 硬碟空間使用率 (Quota)
				...
				break;
            case 8:     // 課程上課統計 (course)
                parent.main.location.href = "sch_course_class_statistics.php";
				break;
            case 9:     // 課程互動
                parent.main.location.href = "co_use_statistics.php";
				break;
            case 10:     // 課程上課統計 (course)
                parent.main.location.href = "co_use_list_statistics.php";
				break;
		}
	}
	...
	function init(evnt) {
		...
		/*
		if ((href.indexOf("sch_statistics.php") !==-1) || ((href.indexOf("sch_login_statistics.php") !==-1)) || ((href.indexOf("sch_course_statistics.php") !==-1))) {
			Calendar_setup("en_begin_date", "%Y-%m-%d", "en_begin_date", false);
			Calendar_setup("en_end_date"  , "%Y-%m-%d", "en_end_date"  , false);
		}
		if (href.indexOf("sch_login_statistics.php") !==-1 || href.indexOf("sch_course_statistics.php") !==-1)
		{
		...
		var obj = null;
		// 開啟工具列視窗 (window)
		var res = parent.catalog.location.toString().match("sch_statistics_tool.php");

		if (res == null) parent.catalog.location.replace("sch_statistics_tool.php");
}
