<?php
	...
	$group_key  = Array('', 'course_id', 'caption', 'sum_to', 'join_sum', 'not_join_sum', 'rate');
						...
						switch ($_POST['target_member']){
								case 1:		// 整所學校
									...
									showXHTML_tr_B('class="cssTrHead"');
										...
										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="schoolSort(3);" title="' . $MSG['Interactive_use_times'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
										...
										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="schoolSort(4);" title="' . $MSG['Participants'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
											echo $MSG['Participants'][$sysSession->lang];
											echo ($sortby == 'quota_limit') ? ($order == 'desc' ? $icon_dn : $icon_up) : '';
											echo '</a>';
										showXHTML_td_E('');

										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="schoolSort(5);" title="' . $MSG['Not_involved'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
											echo $MSG['Not_involved'][$sysSession->lang];
											echo ($sortby == 'quota_used') ? ($order == 'desc' ? $icon_dn : $icon_up) : '';
											echo '</a>';
										showXHTML_td_E('');

										
										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="schoolSort(6);" title="' . $MSG['Participation_rate'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
											echo $MSG['Participation_rate'][$sysSession->lang];
											echo ($sortby == 'rate') ? ($order == 'desc' ? $icon_dn : $icon_up) : '';
											echo '</a>';
										showXHTML_td_E('');
										/*
								...
								case 2:		// 某個課程群組
									...
									showXHTML_tr_B('class="cssTrHead"');
										...
										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="GroupSort(3);" title="' . $MSG['Interactive_use_times'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
											echo $MSG['Interactive_use_times'][$sysSession->lang];
											echo ($gp_sortby == 'sum_to') ? ($gp_order == 'desc' ? $icon_dn : $icon_up) : '';
											echo '</a>';
										showXHTML_td_E('');
										
										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="GroupSort(4);" title="' . $MSG['Participants'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
											echo $MSG['Participants'][$sysSession->lang];
											echo ($gp_sortby == 'join_sum') ? ($gp_order == 'desc' ? $icon_dn : $icon_up) : '';
											echo '</a>';
										showXHTML_td_E('');

										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="GroupSort(5);" title="' . $MSG['Not_involved'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
											echo $MSG['Not_involved'][$sysSession->lang];
											echo ($gp_sortby == 'not_join_sum') ? ($gp_order == 'desc' ? $icon_dn : $icon_up) : '';
											echo '</a>';
										showXHTML_td_E('');

										
										showXHTML_td_B(' align="center" nowrap="noWrap" onclick="GroupSort(6);" title="' . $MSG['Participation_rate'][$sysSession->lang] . '"');
											echo '<a class="cssAnchor" href="javascript:;" onclick="return false;">';
											echo $MSG['Participation_rate'][$sysSession->lang];
											echo ($gp_sortby == 'rate') ? ($gp_order == 'desc' ? $icon_dn : $icon_up) : '';
											echo '</a>';
										showXHTML_td_E('');
										/* 
										showXHTML_td_B(' align="center" nowrap="noWrap" ');
									...
									showXHTML_tr_E('');
									if ($cour_str > 0) {
										$GP_RS = dbGetStMr('WM_term_course','course_id,caption,quota_limit,quota_used,
((select count(*) from WM_qti_exam_test where WM_qti_exam_test.course_id=WM_term_course.course_id and WM_qti_exam_test.type=5) +
(select count(*) from WM_qti_questionnaire_test where WM_qti_questionnaire_test.course_id=WM_term_course.course_id)) as sum_to,
 ((select count(WM_qti_questionnaire_result.exam_id) from WM_qti_questionnaire_test 
 left join  WM_qti_questionnaire_result on WM_qti_questionnaire_test.exam_id=WM_qti_questionnaire_result.exam_id 
 where WM_qti_questionnaire_test.course_id=WM_term_course.course_id)+
 (select count(WM_qti_exam_result.exam_id) from WM_qti_exam_test left join  WM_qti_exam_result on WM_qti_exam_test.exam_id=WM_qti_exam_result.exam_id 
 where WM_qti_exam_test.course_id=WM_term_course.course_id)) as join_sum,
((select count(*) from WM_term_major where WM_term_major.course_id=WM_term_course.course_id)*
((select count(*) from WM_qti_exam_test where WM_qti_exam_test.course_id=WM_term_course.course_id) +
(select count(*) from WM_qti_questionnaire_test where WM_qti_questionnaire_test.course_id=WM_term_course.course_id))-
((select count(WM_qti_questionnaire_result.exam_id) from WM_qti_questionnaire_test 
left join  WM_qti_questionnaire_result on WM_qti_questionnaire_test.exam_id=WM_qti_questionnaire_result.exam_id 
where WM_qti_questionnaire_test.course_id=WM_term_course.course_id)+
(select count(WM_qti_exam_result.exam_id) from WM_qti_exam_test 
left join  WM_qti_exam_result on WM_qti_exam_test.exam_id=WM_qti_exam_result.exam_id 
where WM_qti_exam_test.course_id=WM_term_course.course_id))) as not_join_sum,
IFNULL((((select count(WM_qti_questionnaire_result.exam_id) from WM_qti_questionnaire_test 
 left join  WM_qti_questionnaire_result on WM_qti_questionnaire_test.exam_id=WM_qti_questionnaire_result.exam_id 
 where WM_qti_questionnaire_test.course_id=WM_term_course.course_id)+
 (select count(WM_qti_exam_result.exam_id) from WM_qti_exam_test left join  WM_qti_exam_result on WM_qti_exam_test.exam_id=WM_qti_exam_result.exam_id 
 where WM_qti_exam_test.course_id=WM_term_course.course_id))/(((select count(WM_qti_questionnaire_result.exam_id) from WM_qti_questionnaire_test 
 left join  WM_qti_questionnaire_result on WM_qti_questionnaire_test.exam_id=WM_qti_questionnaire_result.exam_id 
 where WM_qti_questionnaire_test.course_id=WM_term_course.course_id)+
 (select count(WM_qti_exam_result.exam_id) from WM_qti_exam_test left join  WM_qti_exam_result on WM_qti_exam_test.exam_id=WM_qti_exam_result.exam_id 
 where WM_qti_exam_test.course_id=WM_term_course.course_id))+((select count(*) from WM_term_major where WM_term_major.course_id=WM_term_course.course_id)*
((select count(*) from WM_qti_exam_test where WM_qti_exam_test.course_id=WM_term_course.course_id) +
(select count(*) from WM_qti_questionnaire_test where WM_qti_questionnaire_test.course_id=WM_term_course.course_id))-
((select count(WM_qti_questionnaire_result.exam_id) from WM_qti_questionnaire_test 
left join  WM_qti_questionnaire_result on WM_qti_questionnaire_test.exam_id=WM_qti_questionnaire_result.exam_id 
...
(select count(WM_qti_exam_result.exam_id) from WM_qti_exam_test 
left join  WM_qti_exam_result on WM_qti_exam_test.exam_id=WM_qti_exam_result.exam_id 
where WM_qti_exam_test.course_id=WM_term_course.course_id))))*100),0) as rate
','course_id in (' . $cour_str . ') and kind="course" and status!=9' . $sort_cond . $limit_str, ADODB_FETCH_ASSOC);
										if ($GP_RS->RecordCount() > 0) {
											while ($GP_RS1 = $GP_RS->FetchRow()) {
												...
												showXHTML_tr_B($col);
													...
													showXHTML_td('align="left"', $GP_RS1['sum_to']);
													showXHTML_td('align="left"', $GP_RS1['join_sum']);
													showXHTML_td('align="left"', $GP_RS1['not_join_sum']);
													// $used_rate = $GP_RS1['join_sum']/($GP_RS1['join_sum']+$GP_RS1['not_join_sum'])*100;
													showXHTML_td('align="left" nowrap', round($GP_RS1['rate'],2) . ' %');
												showXHTML_tr_E('');
											}
										}
									}else{
