<?php
/* $Id: common.lib.php,v 1.1 2010/02/24 02:38:24 saly Exp $ */
 ...
 && (! isset($_COOKIE['pmaCookieVer'])
  || $_COOKIE['pmaCookieVer'] < $pma_cookie_version)) {
    ...
    foreach($_COOKIE as $cookie_name => $tmp) {
        if ($cookie_name == 'idx' || $cookie_name == 'school_hash')
            $wm_preserveds[$cookie_name] = $tmp;
        else
        	PMA_removeCookie($cookie_name);
    }
    $_COOKIE = $wm_preserveds;
...
// ¾ã¦X WM pro
require_once($_SERVER['DOCUMENT_ROOT'] . '/config/phpMyAdmin.inc.php');
