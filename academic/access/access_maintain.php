<?php
	...
	function scope_view($val){
		static $scopes = array(1 => 'S',
						       2 => 'T',
						       4 => 'D',
						       8 => 'M');
		$ret = '';
		foreach($scopes as $k => $v) $ret .= ($val & $k) ? $v : '.';
		return $ret;
	}
	...
	list($total_item) = dbGetStSr('WM_acl_function', 'count(*)', '1', ADODB_FETCH_NUM);
	$total_page = max(1, ceil($total_item / sysPostPerPage));
	$cur_page = intval($_GET['page']);
	if ($cur_page > $total_page) $cur_page = $total_page;
	elseif ($cur_page < 1) $cur_page = 1;

	$sorts = array('', 'function_id', 'caption', 'scope', 'default_permission');
	$sort = intval($_GET['sort']);
	if ($sort > 4 || $sort < 1) $sort = 1;
...
function sort(v){
	location.replace('access_maintain.php?page={$cur_page}&sort=' + v);
}

function page(v){
	var url = 'access_maintain.php?sort={$sort}&page=';
	switch(v){
		case -1:
			location.replace(url + 1);
			break;
		case -2:
			location.replace(url + ({$cur_page} - 1));
			break;
		case -3:
			location.replace(url + ({$cur_page} + 1));
			break;
		case -4:
			location.replace(url + {$total_page});
			break;
		default:
			location.replace(url + v);
			break;
	}
}

function process(v){
	var procForm = document.getElementById('tabsSet1');
	switch(v){
		case 1:
			if (!confirm('Are you sure to remove those functions ?')) return;
			procForm.op.value = 'remove';
			procForm.submit();
			break;
		case 2:
			procForm.op.value = 'modifyScope';
			procForm.submit();
			break;
		case 3:
			procForm.op.value = 'modifyPermission';
			procForm.submit();
			break;
	}
}
...
EOB;
	...
	showXHTML_head_E();
	showXHTML_body_B('onload="document.getElementById(\'toolbar2\').innerHTML=document.getElementById(\'toolbar1\').innerHTML;"');
	  showXHTML_table_B('border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse"');
	    showXHTML_tr_B();
	      showXHTML_td_B();
		$ary = array(array($MSG['function_maintain'][$sysSession->lang], 'tabsSet1',  ''),
			     array($MSG['add_function'][$sysSession->lang],      'tabsSet2',  'tabsSet2.style.display="inline";')
			    );
	    ...
	    showXHTML_tr_B();
	      showXHTML_td_B('');
		showXHTML_form_B('action="access_process.php" method="POST" style="display:inline"', 'tabsSet1');
		  showXHTML_table_B('border="0" cellpadding="3" cellspacing="1" style="border-collapse: collapse" class="cssTable"');

		    showXHTML_tr_B('class="cssTrEvn"');
		      showXHTML_td_B('colspan="8" id="toolbar1"');

		      showXHTML_input('button', 'btnSel1', $MSG['select_all'][$sysSession->lang], '', 'id="btnSel1" onclick="selfunc()" class="cssBtn"');

		      	echo $MSG['page_num'][$sysSession->lang];
				showXHTML_input('select', '', array_range(1, $total_page), $cur_page, 'onchange="page(this.value);"');
				showXHTML_input('button', '', $MSG['first_page'][$sysSession->lang], '', 'class="cssBtn" ' . ($cur_page==1          ?'disabled' : 'onclick="page(-1);"'));
				showXHTML_input('button', '', $MSG['prev_page'][$sysSession->lang] , '', 'class="cssBtn" ' . ($cur_page==1          ?'disabled' : 'onclick="page(-2);"'));
				showXHTML_input('button', '', $MSG['next_page'][$sysSession->lang] , '', 'class="cssBtn" ' . ($cur_page==$total_page?'disabled' : 'onclick="page(-3);"'));
				showXHTML_input('button', '', $MSG['last_page'][$sysSession->lang] , '', 'class="cssBtn" ' . ($cur_page==$total_page?'disabled' : 'onclick="page(-4);"'));
				echo '&nbsp;&nbsp;';
				showXHTML_input('button', '', $MSG['remove'][$sysSession->lang],          '', 'class="cssBtn" onclick="process(1);"');
				showXHTML_input('button', '', $MSG['batch_mdy_scope'][$sysSession->lang], '', 'class="cssBtn" onclick="process(2);"');
				showXHTML_input('button', '', $MSG['batch_mdy_defs'][$sysSession->lang],  '', 'class="cssBtn" onclick="process(3);"');
		      showXHTML_td_E();
		    showXHTML_tr_E();
		    showXHTML_tr_B('class="cssTrHead font01"');
				...
				showXHTML_td_E();
		      	...
		      	showXHTML_td('width="80"  align="center"', $MSG['serial_no'][$sysSession->lang]);
		      	showXHTML_td('width="100" align="center"', '<a href="javascript:sort(1)">' . $MSG['function_id'][$sysSession->lang]  . '</a>');
		      	showXHTML_td('width="360" align="center"', '<a href="javascript:sort(2)">' . $MSG['func_caption'][$sysSession->lang] . '</a>');
		      	showXHTML_td('width="80"  align="center"', '<a href="javascript:sort(3)">' . $MSG['scope'][$sysSession->lang]        . '</a>');
		      	showXHTML_td('width="110" align="center"', '<a href="javascript:sort(4)">' . $MSG['default_permission'][$sysSession->lang] . '</a>');
		      	showXHTML_td('width="35"  align="center"', '&nbsp;');
		      	showXHTML_td('width="35"  align="center"', '&nbsp;');
		    showXHTML_tr_E();

	$RS = dbGetStMr('WM_acl_function', 'function_id,caption,scope+0 as scope,default_permission+0 as def', '1 order by ' . $sorts[$sort] . ' limit ' . ($cur_page-1)*sysPostPerPage . ',' . sysPostPerPage, ADODB_FETCH_ASSOC);
	$i = (($cur_page-1)*sysPostPerPage) + 1;
	while(!$RS->EOF){
		$col = $col == 'class="cssTrEvn font01"' ? 'class="cssTrOdd font01"' : 'class="cssTrEvn font01"';
		    showXHTML_tr_B($col);
		      showXHTML_td_B('align="center"');
		      	showXHTML_input('checkbox', 'fid[]', $RS->fields['function_id'],'','onclick="chgCheckbox()"');
		      showXHTML_td_E();
		      showXHTML_td('align="center"', $i++);
		      showXHTML_td_B();
		      	echo '<a href="javascript:;" onclick="this.href=\'access_modifyfunction.php?fid=', $RS->fields['function_id'],'&', $_SERVER['QUERY_STRING'], '\';" title="', $MSG['msg_viewAndModiCont'][$sysSession->lang], '">', $RS->fields['function_id'], '</a>';
		      showXHTML_td_E();
                      
                      switch($sysSession->lang) {
                          case 'GB2312':
                              $caption = big2gb($RS->fields['caption']);
                              break;
		      ...
		      showXHTML_td('nowrap', '<span style="width: 360px; overflow: hidden">' . $caption . '</span>');
		      showXHTML_td('align="center" style="font-size: 16px"', scope_view(intval($RS->fields['scope'])));
		      showXHTML_td('align="center" style="font-size: 16px"', default_permission_view(intval($RS->fields['def'])));
		      showXHTML_td('', sprintf('<a href="javascript:;" onclick="this.href=\'access_acl.php?fid=%s&%s\';" title="%s">Add</a>', $RS->fields['function_id'], $_SERVER['QUERY_STRING'], $MSG['msg_addACL'][$sysSession->lang]));
		      showXHTML_td('', sprintf('<a href="javascript:;" onclick="this.href=\'access_acllist.php?fid=%s&%s\';" title="%s">List</a>', $RS->fields['function_id'], $_SERVER['QUERY_STRING'], $MSG['msg_listACL'][$sysSession->lang]));
		    showXHTML_tr_E();

		$RS->MoveNext();
	}
		    showXHTML_tr_B('class="cssTrEvn"');
		      showXHTML_td('colspan="8" id="toolbar2"', '&nbsp;');
		    showXHTML_tr_E();
		  showXHTML_table_E(); //t2
		  showXHTML_input('hidden', 'op');
		  showXHTML_input('hidden', 'attributes', $_SERVER['QUERY_STRING']);
		showXHTML_form_E();

		showXHTML_form_B('action="access_addfunction.php" method="POST" onsubmit="return (this.elements[0].value ? true : false);" style="display: none"', 'tabsSet2');
		  showXHTML_table_B('border="0" cellpadding="3" cellspacing="1" width="760" style="border-collapse: collapse" class="box01"');
		    ...
		    showXHTML_tr_B('class="cssTrEvn font01"');
		      ...
		      showXHTML_td_B();
		        showXHTML_input('text', 'function_id', '', '', 'class="box02"');
		      showXHTML_td_E();
		    ...
		    showXHTML_tr_B('class="cssTrOdd font01"');
		      ...
		      showXHTML_td_B();
		        showXHTML_input('text', 'caption', '', '', 'class="box02" size="50" maxlength="254"');
		      showXHTML_td_E();
		    ...
		    showXHTML_tr_B('class="cssTrOdd font01"');
		      showXHTML_td('', $MSG['default'][$sysSession->lang]);
		      showXHTML_td_B();
			...
			showXHTML_input('submit', '', $MSG['add_function'][$sysSession->lang], '', 'class="cssBtn"');
		      showXHTML_td_E();
		    showXHTML_tr_E();
