<?php
	 ...
	 * @version     CVS: $Id: stud_mail1.php,v 1.1 2010/02/24 02:40:31 saly Exp $
	...
	if (is_array($_POST['user']) && count($_POST['user'])) {	// 取得勾選學員的Email
		...
		$reciver = $sysConn->GetCol('select email from WM_user_account where username in ("'.implode('","', $user).'") and email is not null');
	}
	...
	$mail->uri_target  = defined('URI_TARGET') ? URI_TARGET : 'stud_info.php';
	$mail->memsg['btn_return'] = $MSG['return_cour_stat'][$sysSession->lang];
	$mail->send();
?>
