		...
		switch(nodes.item(i).tagName){
			case 'item':
                ...
                if (insertPosition[1] == nodes.item(i).getAttribute('identifier')) {
                    ...
                    insertNew = '<div class="icon-new-tag"></div>';
                    itemCnt++;
                }
                nodeIntro    = ((iref = nodes.item(i).getAttribute('identifierref')) &&
					 (res = resources.selectSingleNode('./resource[@identifier="' + iref + '"]')) &&
					 (nodeIntro = res.getAttribute('intro')) &&
					 (nodeIntro != 'undefined')) ? nodeIntro : '';
                IH +=   '<div class="box2 selector2">' +
                            '<div class="title" onclick="selectSection(this);" style="max-width: 75%;" title="' + getTitle(nodes.item(i)) + '">' +
                                '<input type="checkbox" name="' + idValue + '" id="radio-'+(itemCounter)+'" style="display: none;"/>' +
                                 insertNew + MSG_UNIT_ORDER.replace('%num%', (itemCounter++)) + ': ' + getTitle(nodes.item(i)) +
                            '</div>' +
                            '<div class="operate">' +
                                '<div class="icon-edit" onclick="showLCMS(\'' + idValue + '\', \'edit\');" style="cursor: pointer;"></div>' +
                                '<div class="icon-delete" onclick="selectSection(this.parentNode); executing(4);" style="cursor: pointer;"></div>' +
                            '</div>' +
                            '<div class="content" onclick="showLCMS(\'' + idValue + '\', \'preview\');" style="cursor: pointer;">' +
                                '<div class="data1">' +
                                    '<div class="content">'+nodeIntro + 
                                    '</div>' +
                                '</div>' +
                            '</div>' +
                            '<button class="btn-plane-white" onclick="selectSection(this); selectLcmsContent(); return false;"><div class="icon-insert-s"></div>'+MSG_INSERT_UNIT+'</button>' +
                        '</div>';
...
function displayLayout(){
	...
	if (isEnableSSS && false) {
        ...
        // 插入第一個節點前的按鈕
        var firstInsertBtn = '<button class="btn-plane-white" style="margin-top: 15px;" onclick="insertFirst = true; selectLcmsContent(); return false;"><div class="icon-insert-s"></div>插入單元</button>';
		var IH = firstInsertBtn + /* getTitle(organization) + '<br>' + */getChildItem(organization);
	}
...
function newNodeInit(newNode, extra){
    ...
    if (extra === undefined) {
        ...
        nNode.setAttribute('intro', extra.intro);
    }
...
function executing(idx){
	...
	switch(idx){
		...
		case  5: // 存檔
			...
			var xmlHttp = XmlHttp.create();
			xmlHttp.open('POST', 'cour_path_save.php?cid='+MOD_COURSE_ID+'&typep=3&num='+asmtNum, false);
			xmlHttp.send(xmlDoc);
...
function confirmLcmsImport() {
    ...
    for (i = 0, c = lcmsData.unit.length; i < c; i += 1) {
        ...
        newNodeInit(newNode.parentNode, {
            ...
            intro: lcmsData.unit[i].extra.intro
        });
    }
...
function selectLcmsContent() {
    ...
    window.open('lcms.php?action=import&type=unit', 'lcmsDialog', 'height=650,width=1000,resizable=1,scrollbars=1');
}
...
function showLCMS(id, type) {
    ...
    if (type == 'preview') {        
        window.open(resource.getAttribute('href')+"/assetfull", 'lcmsDialog', 'top=0,left=0,width=1100,height=750,location=no,scrollbars=yes');
    } else if (type == 'edit') {
        window.open((resource.getAttribute('href')).replace('view', 'edit') + '/1', 'lcmsDialog', 'top=0,left=0,width=1100,height=750,location=no,scrollbars=yes');
    }
}
