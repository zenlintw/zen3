<?php
	...
	$ticket = md5(sysTicketSeed . 'setAnnt' . $_COOKIE['idx']);
	...
	if (!empty($nid)) {
		...
		$dd['mailfollow'] = ($ps === false) ? (($RS['switch'] === null)?'yes':'no') : 'yes';
	...
	function saveSetting() {
        ...
        */
        ...
        $.ajax({
            ...
            'type': 'POST',
            dataType:"html",
            'data': $('#setFm').serialize(),
            'success': function (res) {
                if (res == '{$MSG['access_deny'][$sysSession->lang]}') {
                    alert('{$MSG['access_deny'][$sysSession->lang]}');
                } else {
                    var wrappedObj = $("<code></code>").append($(res));
                    var table = $("#tabs_host", wrappedObj);
                    alert(table.find('.cssTrHead').children().html());
                }
            },
            'error': function () {
                alert('ajax fail!');
	...
	function statDateShow(val, objName) {
        ...
        if(!val) {
            $("#"+objName).find('input').val('');
        } else {
            $("#"+objName).find('input').val('{$now}');
        }
	}
    ...
    */
	...
	window.onload = function () {
        ...
        statListShow("visible");
		/* timerID = setInterval("chgTitle(document.setFm.help)",500); */
	};
        
    function go_manage() {
        location.replace("m_cour_annt.php");
    }
BOF;
	...
	showXHTML_body_B('');
		...
		if (empty($_POST['nid'])) {
			...
			$ary[] = array($MSG['management'][$sysSession->lang] , 'tab_annt', 'go_manage()');
            $ary[] = array($MSG['setting'][$sysSession->lang]    , 'tab_setting');
            // $ary[] = array($MSG['subject_title_property'][$sysSession->lang], 'tabs_host');
		}
        echo '<div align="center">';
		showXHTML_tabFrame_B($ary, 2, 'setFm', '', 'action="cour_subject_save.php" method="post" enctype="multipart/form-data" style="display: inline;"', false);
			// 議題編號
			...
			showXHTML_table_B('width="600" border="0" cellspacing="1" cellpadding="3" id="tabs_host" class="cssTable"');
				...
				showXHTML_tr_B($col);
					showXHTML_td('', $MSG['display_name'][$sysSession->lang]);
					showXHTML_td_B();
                ...
                */
				// 顯示或隱藏
                showXHTML_input('hidden', 'visibility', 'visible');
                /*
				$col = ($col == 'class="cssTrEvn"') ? 'class="cssTrOdd"' : 'class="cssTrEvn"';
				showXHTML_tr_B($col);
                ...
                */
				...
				showXHTML_tr_B($col);
					showXHTML_td_B('colspan="2" align="center"');
                        showXHTML_input('button', '', $MSG['msg_ok'][$sysSession->lang], '', 'class="cssBtn" onclick="saveSetting()"');
