<?php
	 ...
	 * @since   2004/01/16
	 * @author  ShenTing Lin
	 * @version $Id: cour_subject_delete.php,v 1.1 2010/02/24 02:40:23 saly Exp $
	...
	foreach ($nid as $val) {
		...
		$val = intval($val);
		list($bid) = dbGetStSr('WM_term_subject', '`board_id`', "`node_id`={$val}", ADODB_FETCH_NUM);
		...
		if ($sysConn->Affected_Rows() > 0) {
			...
			// 刪除議題
			dbDel('WM_term_subject', "node_id='{$val}'");
