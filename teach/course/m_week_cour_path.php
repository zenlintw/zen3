<?php
    ...
    if (sysLcmsEnable) {
        if (($cour_ptype != 0 && $cour_ptype != 3 && !$isNullContent)) {
...
");
    ...
    $lcmsEnable = sysLcmsEnable ? 'true' : 'false';
  	...
  	$scr = <<< EOB
var MSG_DELETE  = "{$MSG['msg_unit_delete'][$sysSession->lang]}";
...
var MSG_INSERT_UNIT = "{$MSG['insert_unit'][$sysSession->lang]}";
...
function radioChoice(e) {
    $(".box2.selector").removeClass("slt-active");
    if ($(e).attr('checkSelect') == 'Y') {
        $(e).attr('checked', false);
        $(e).attr('checkSelect', 'N');
    } else {
        $("input[type=radio][name^='I_SCO_']").each(function () {
            $(this).attr('checked', false);
            $(this).attr('checkSelect', 'N');
        });
        $(e).parent().parent().addClass("slt-active");
        $(e).attr('checked', true);
        $(e).attr('checkSelect', 'Y');
    }
};
*/
...
function selectSection(obj) {
    /* 點選標記橘色 */
    $(".selector2 .title").removeClass("selected");
    $(obj).parent().find('.title').addClass("selected");
    /* 選取 checkbox */
 ...
 */
    ...
    showXHTML_script('include', '/teach/course/m_week_cour_path.js');
    unset($scr);
    ...
    echo '<div class="box1">
        ...
        <div class="content">
            ...
            <div id="no-data-msg" class="data4" style="display: none;">
                <div class="message">'.
                    $MSG['msg_no_unit'][$sysSession->lang] .
                '</div>
                <div class="buttons">
                    <button class="btn btn-blue" onclick="selectLcmsContent();">'.
                        $MSG['new_unit'][$sysSession->lang] .
                    '</button>';
                    if ($cour_ptype == 0) {
