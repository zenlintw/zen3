	...
	if (ereg('^[0-9]{9}$', $_POST['hwid']))   // 顯示頁面
	{
		...
		*/
		...
		if(empty($_POST['examinee'])){
			...
			$file_path = sysDocumentRoot . sprintf('/base/%5u/course/%8u/homework/A', $sysSession->school_id, $sysSession->course_id);
			
		    exec("cd {$file_path}  && rm -rf temp_{$_POST['hwid']} && cp -lr {$_POST['hwid']} temp_{$_POST['hwid']}");
		    
		    $file_path = $file_path.'/temp_'.$_POST['hwid'];
		    $dir = opendir($file_path);
		    while (($file = readdir($dir)) !== false)
		        {
			    if ($file!='..' && $file!='.') {
			    	$user = getUserDetailData($file);
			    	exec("cd {$file_path} && mv '{$file}' '{$file}_{$user['realname']}'");
			    }
			}
		    closedir($dir);
		}else{
			$fname = 'hw' . $_POST['examinee'] . '.zip';
			$file_path = sysDocumentRoot . sprintf('/base/%5u/course/%8u/homework/A/%09u/%s/', $sysSession->school_id, $sysSession->course_id, $_POST['hwid'], $_POST['examinee']);
		...
		$options='-D -r -q -7';
		if(empty($_POST['examinee'])){
			$mv_path = sysDocumentRoot . sprintf('/base/%5u/course/%8u/homework/A/%09u/', $sysSession->school_id, $sysSession->course_id, $_POST['hwid']);
		    exec("cd {$file_path} && {$program} {$options} Coursetemp.zip . && mv Coursetemp.zip {$mv_path}");
			if (is_dir($file_path)) {
	        	exec("rm -rf {$file_path}");
	        }
		} else {
            exec("/bin/rm {$file_path}Coursetemp.zip");
            exec("/bin/rm {$file_path}zi*");
