		...
		<table border="0" cellpadding="3" cellspacing="1" width="960" style="border-collapse: collapse" class="box01">
			<tr class="bg02 font01">
				<td class="cssTd" colspan="3" align="right">
					<input type="button" value="{$MSG['cancel'][$sysSession->lang]}" class="button01" onclick="location.replace('exam_maintain.php');"/>
					<input type="button" value="{$MSG['prev_step'][$sysSession->lang]}" class="button01" onclick="switchTab(0);"/>
					<input type="button" value="{$MSG['next_step'][$sysSession->lang]}" class="button01" onclick="switchTab(4);"/>
				</td>
			</tr>
			...
			<tr class="bg04 font01">
				...
				<td class="cssTd" colspan="2">
					<input type="checkbox" name="immediate_random_pick" value="immediate_random_pick" onclick="randomCheck2(this);" style="display: none;" checked="checked"/>{$MSG['total'][$sysSession->lang]}
				...
				<td class="cssTd" colspan="2">
					...
					<script type="text/javascript">
						iniParagraph.push([
							...
							'<xsl:value-of select="generate-id(version)" />'
						]);
					</script>
                                        -->
				</td>
						...
						<tr>
							...
							<td>
								<xsl:element name="input">
									...
									<xsl:attribute name="onclick">if (this.checked) parentNode.previousSibling.firstChild.checked=true;</xsl:attribute>
			...
			<tr class="bg03 font01">
				<td class="cssTd" colspan="3">
					...
					<xsl:element name="input">
						...
						<xsl:attribute name="onblur">calculation(this)</xsl:attribute>
						<xsl:attribute name="onkeyup">this.value=this.value.replace(/[^\d]/,'')</xsl:attribute>
					</xsl:element>
