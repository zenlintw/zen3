<?php
    ...
    //ACL begin
    if (QTI_which == 'exam') {
        $sysSession->cur_func='1600200400';
    }
        ...
        in_array($_POST['batchPublish'], array('revised', 'all')))
    {
        ...
        {
            $result_rs = dbGetStMr('WM_qti_homework_result', 'examinee,time_id,content', 'exam_id=' . intval($_SERVER['argv'][1]) . ' and status != "revised" and status != "publish"');
            if ($result_rs && $result_rs->RecordCount()) 
            {
                ...
                while ($row = $result_rs->FetchRow())
                {
                    ...
                    if (preg_match_all('/<input\b[^>]*\bname="item_scores\b[^>]*\bvalue="(-?[0-9.]*)"/', $result_html, $regs))
                        ...
                        $total_score = 0;
                    dbSet('WM_qti_homework_result', "score={$total_score}, status='revised'" , "exam_id={$_SERVER['argv'][1]} and examinee='{$row['examinee']}' and time_id={$row['time_id']}");
                    reCalculateQTIGrade($row['examinee'], $_SERVER['argv'][1], 'homework', null, isset($team_id) && $team_id > 0 ? $team_id : null);
                }
                reCalculateGrades();
            }
        }
        
        $result = (bool) dbSet('WM_qti_homework_result', 'status="publish"', 'exam_id=' . intval($_SERVER['argv'][1]) . ' and status="revised"');
    ...
    if (QTI_which == 'homework' && isAssignmentForGroup($_SERVER['argv'][1]))
    {
        if ($team_id)
        {
            $sqls = 'select distinct D.group_id, D.team_id, G.caption,R.status ' .
                    ' from WM_student_div as D ' .
                    ' left join WM_qti_homework_result as R on D.username=R.examinee and R.exam_id=' . $_SERVER['argv'][1] .
    ...
    else
    {
        ...
        $sqls            = 'select M.username, sum(if(isnull(R.status),0,1)) as total, ' .
                           'sum(if(R.status="submit",1,0) or if(R.status="break",1,0) or if(R.status="chgWin",1,0))  as uncorrected ' .
...
function correct_import(ticket, hwid){
    parent.rtop.location.replace('exam_correct_import.php?' + ticket + '+' + hwid);
    parent.rbottom.document.body.innerHTML='';
    parent.rbottom.document.body.style.background='#F6F6F6';
}
...
function HW_TarAttaches(title,hwid)
{
    ...
    {
        window.open('/teach/homework/TarAttach.php?'+hwid, 'winTar', "width=350,height=80,resizable=0,status=0,titlebar=0,toolbar=0,menubar=0,scrollbars=0");
    }
}
...
EOB;
    ...
    if (QTI_which == 'homework' && isAssignmentForGroup($_SERVER['argv'][1]))
    {
            showXHTML_tr_B('class="bg02 font01"');
              ...
              if (QTI_which == 'homework'){
                ...
                showXHTML_td('align="center" nowrap', $MSG['examinee_title4'][$sysSession->lang]);
              }
        ...
        if (is_array($RS) && count($RS))
            foreach($RS as $group_id => $fields)
            {
                ...
                {
                    showXHTML_tr_B($col . ' onmouseover="chBgc(this, true);" onmouseout="chBgc(this, false);"' . (($fields['status'] && $fields['status'] != 'break') ? (' onclick="pickUser(\'' . $group_id . '+' . $fields['team_id'] . "', '" . md5($ticket_head . $group_id) . '\');" style="cursor: pointer'):'" style="cursor: default') . '"');
    ...
    if (QTI_which == 'homework'){ // 作業下載
        $hw_path = sysDocumentRoot . sprintf('/base/%5u/course/%8u/homework/A/%09u', $sysSession->school_id, $sysSession->course_id, $_SERVER['argv'][1]);
        if (is_dir($hw_path) && (bool)exec("find '$hw_path' -type f | wc -l"))
        {
            ...
            showXHTML_tr_B('class="bg03 font01"');
                showXHTML_td_B('colspan="6" align="center"');
                    showXHTML_input('button', '', $MSG['hw_import'][$sysSession->lang]   , '', 'id="import" class="cssBtn" onclick="correct_import(\'' . md5($ticket_head) . '\',\'' . $_SERVER['argv'][1] . '\');"');
                showXHTML_td_E('');
            showXHTML_tr_E();
        }
    }
