...
class GetSplashAction extends baseAction
{
    function main()
    {
        // 驗證 Ticket
        ...
        if (in_array($device, $devicePermit)) {
            if ($device === 'PHONE') {
                // iphone 圖片檔案路徑
                $pictureFile = getImageFilename('splash-iphone', $sysSession->school_id);
            } else if ($device === 'TABLET') {
                // ipad 圖片檔案路徑
                $pictureFile = getImageFilename('splash-ipad', $sysSession->school_id);
            }
