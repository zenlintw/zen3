...
class CourseNodesResourcesAction extends baseAction
{
    ...
    function main()
    {
        ...
        // 取得參數資料，並作json解譯為array格式
        $inputData = file_get_contents('php://input');
        $nodesData = JsonUtility::decode($inputData);

        // 教材路徑前置參數
        $downloadPathParent = WM_SERVER_HOST;
        $relativePathParent = '/base/'.$sysSession->school_id.'/course/%s/content/%s';

        foreach ($nodesData as $courseId => $scoIdData) {
            $scoIdCount = count($nodesData[$courseId]);
            $where = "`course_id` ={$courseId} ORDER BY `serial` DESC";
            $content = dbGetOne('`WM_term_path`', '`content`', $where);

            if (!empty($content)) {
                $xmlDoc = domxml_open_mem($content);
                $ctx = xpath_new_context($xmlDoc);

                // 1個課程1個array，故先unset，避免array裡的資料重複
                unset($data);

                for ($i = 0; $i < $scoIdCount; $i++) {
                    $scoId = $nodesData[$courseId][$i];
                    $data[$scoId] = array();
                    ...
                    for ($j=0; $j<count($resources); $j++) {
                        $filePath = $resources[$j]['file'];
                        $fileHref = $resources[$j]['url'];
                        ...
                        $scoData['download_path'] = $fileHref;
                        $scoData['relative_path'] = $filePath;
                        $scoData['update_datetime'] = $updateDatetime;
                        $scoData['size'] = $fileSize;
                        $scoData['metadata'] = '';

                        array_push($data[$scoId], $scoData);
